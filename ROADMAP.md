...
| Liquidity Sweep Reversal strategy (from LuxAlgo concept) | 🟡 built, needs validation | `scripts/liquidity_sweep_reversal.pine`. Distills LuxAlgo Liquidity Delta Profiler's REJ/ABS sweep-reversal signal into a real strategy with LTF-delta confirmation, real stops/targets/sizing. Replaces the indicator's fake time-based "win rate" with a true backtest. Needs TradingView Premium validation. |
