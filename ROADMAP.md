...
| Day 2+ Continuation (Assan S3) | ✅ built | `backtest/strategies/day2_continuation.py`. Best per-trade edge in project (+0.51 R momentum15, +0.32 R SP500) BUT portfolio max DD -46% (worse than SPY). Regime filter tested — didn't fix DD. Still doesn't beat SPY. Pine version not yet written. |
| MTF Structure + Value (hybrid, order-flow request) | 🟡 built, needs validation | `scripts/mtf_structure_value.pine`. Daily HTF / hourly entry. Codeable parts (HTF dir, VWAP, S&D zones, approx POC, structure) mechanical; footprint/delta/true-POC are discretionary (not accessible in Pine). Needs TradingView Premium intraday validation. |
| Order-flow data boundary documented | ✅ | Pine cannot access footprint, signed delta, or true Volume Profile POC. These stay discretionary or require Sierra Chart / Bookmap / Databento MBO + Python. CORRECTION: approximated delta IS possible via request.security_lower_tf (intrabar volume by sub-bar direction) — lower fidelity than true footprint but codeable. |
| Liquidity Sweep Reversal strategy (from LuxAlgo concept) | 🟡 built, validated marginal | `scripts/liquidity_sweep_reversal.pine`. NVDA 1H real backtest: 54 trades, 42.6% WR, PF 1.255, +2.86% (~2%/yr). Statistically real but economically marginal; LTF-delta confirmation prevented it being a loser (unlike backside short). Indicator's displayed WR was the mirage. |
| VWAP deviation bands paired into avwap_pullback (Order Flow VWAP concept) | ✅ tested | `use_vwap_bands` option in avwap_pullback.py using rolling-VWAP σ-band. A/B (SP500/10y): bands ON = 56.5% WR (+9pp) & half the DD, but SAME +0.06 avg R. Risk-profile improvement, not edge. Cycle-anchored VWAP band was unreachable (0 trades) — had to use rolling VWAP, documented. |
| Delta-accumulation filter on consolidation_breakout (LuxAlgo synthesis) | ✅ tested (proxy) | `use_delta_filter` in consolidation_breakout.py. Hypothesis: base accumulating buy-delta breaks out better. A/B (SP500/10y): proxy delta filter slightly HURTS (0.31→0.28 R, fewer trades, flat WR). Proxy delta carries no breakout-prediction info. Pine version with REAL LTF delta (`i_use_delta`) built for TradingView Premium validation — final honest test. |
| LuxAlgo scripts reviewed (5) | ✅ | Liquidity Delta Profiler (fake WR), Order Flow VWAP (honest, σ-bands useful), kNN Market Architecture (ML is a no-op), Dynamic Delta FVG (real LTF delta, best-built), Range Intelligence Suite (consolidation+delta). Only genuinely-new signal across all = real LTF delta; proxy versions add nothing. |
| Market-cap tier test (Path 2) | ✅ refuted | Edge DECREASES with smaller cap: consolidation_breakout +0.30 (large) → +0.17 (mid) → +0.06 (small). Small caps noisier + survivorship-biased + higher friction. Keep strategies in large caps. Added sp400/sp600 universes + fetch_index.py. |
| Discretionary playbook (Path 3) | ✅ | `docs/discretionary_playbook.md` — the honest operating manual: machine funnels + sizes candidates, human supplies order-flow judgment. Weekly routine, per-candidate review, risk rules, what-not-to-do, journal. The toolkit's actual deliverable. |
| Smart Money Concepts strategy test (tech/auto focus) | ✅ no-edge | `smc_structure.py` (BOS/CHoCH core) + `tech_auto` universe. tech_auto +0.28 R, SP500 +0.21 R. Portfolio vs SPY: 10.2% vs 15.7% CAGR, no DD/Sharpe edge. SMC = crowded price geometry on the hardest table = no edge. Confirms table-selection thesis. The edge in tech/auto, if any, must be domain/fundamental — SMC is only a timing overlay. |
| Lesson: indicator win-rate displays | ✅ documented | LuxAlgo's top-right "win rate" counts a win as price ticking favorably for N bars with NO stop/target/costs — not tradeable. Documented in docs/strategies/liquidity_sweep_reversal.md as the canonical example. |
